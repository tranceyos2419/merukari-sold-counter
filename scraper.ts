...
export const scrapeOMURL = async (
    ...
    try {
      ...
      for (const item of uniqueItems) {
        ...
        if (
          ...
          processedItemIds.add(item.id); // Mark item as processed
          MSC++;
  ...
  while (retries < retryLimit) {
    ...
    if (retries >= retryLimit) {
      console.error(
        `${identity} | Failed to scrape OMURL ${retryLimit} attempts.`,
      );
    } else {
      console.warn(
        `${identity} | Retrying OMURL scrape (${retries}/${retryLimit})...`,
      );
      await wait(waitTime);
    }
  }
...
export const scrapeNMURL = async (
  ...
  while (retries < retryLimit) {
    ...
    if (retries >= retryLimit) {
      console.error(
        `${identity} | Failed to scrape NMURL ${retryLimit} attempts.`,
      );
    } else {
      console.warn(
        `${identity} | Retrying NMURL scrape (${retries}/${retryLimit})...`,
      );
      await wait(waitTime);
    }
  }
